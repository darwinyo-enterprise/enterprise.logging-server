...
namespace Enterprise.LoggingServer.Tests
{
    ...
    public class DebugLogControllerTest : BaseTest
    {
        public DebugLogControllerTest(AuthorizationServiceFixture authorizationServiceFixture, LoggingServiceFixture loggingServiceFixture)
        {
        ...
        {
            var datas = (List<DebugLog>)_loggingServiceFixture.LoggingServices.GetAllDebugLogAsync(accessToken).Result;
            if (datas.Count > 0)
            {
                _loggingServiceFixture.LoggingServices.DeleteAllDebugLogAsync(accessToken).Wait();
            }
        }
        ...
        {
            // Pre Test
            var datas = _loggingServiceFixture.LoggingServices.GetAllDebugLogByDateAsync(DateTime.Today, accessToken).Result;
            Assert.Empty(datas);

            // Action
            _loggingServiceFixture.LoggingServices.LogAsync(_logModelMockData.GetDebugLogMockData(),IsDevMode: false).Wait();

            // Assert
            datas = _loggingServiceFixture.LoggingServices.GetAllDebugLogByDateAsync(DateTime.Today, accessToken).Result;
        ...
        {
            // Pre Test
            List<DebugLog> datas = (List<DebugLog>)_loggingServiceFixture.LoggingServices.GetAllDebugLogByDateAsync(DateTime.Today, accessToken).Result;
            Assert.Empty(datas);
            ...
            // Assert Item Exists
            datas = (List<DebugLog>)_loggingServiceFixture.LoggingServices.GetAllDebugLogByDateAsync(DateTime.Today, accessToken).Result;
            Assert.NotEmpty(datas);

            // Assert
            var response = _loggingServiceFixture.LoggingServices.GetDebugLogByIDAsync(datas[0].LogID.ToString(), accessToken).Result;
            Assert.True(LogComparer<DebugLog>.Compare(datas[0], response));
        }
        [Fact]
        ...
        public void Delete_WhenIDParamaterPass_DeleteSingleLogs()
        {
            ...
            List<DebugLog> datas = (List<DebugLog>)_loggingServiceFixture.LoggingServices.GetAllDebugLogAsync(accessToken).Result;
            Assert.NotEmpty(datas);
            ...
            // Action
            _loggingServiceFixture.LoggingServices.DeleteDebugLogByIDAsync(searchedID, accessToken).Wait();

            // Assert Item Exists
            datas = (List<DebugLog>)_loggingServiceFixture.LoggingServices.GetAllDebugLogAsync(accessToken).Result;
            Assert.NotEmpty(datas);
            ...
            // Assert Data Deleted
            var data = _loggingServiceFixture.LoggingServices.GetDebugLogByIDAsync(searchedID, accessToken).Result;
            Assert.Null(data);
        }
        [Fact]
        public void Delete_WhenIDParamaterPassNotExists_DeleteSingleLogs()
        {
            ...
            var datas = _loggingServiceFixture.LoggingServices.GetAllDebugLogAsync(accessToken).Result;
            Assert.Empty(datas);
            ...
            var response = _loggingServiceFixture.LoggingServices.DeleteDebugLogByIDAsync(searchedID, accessToken).Result;
        ...
        public void Delete_WhenItemNotExists_ThrowItemNotFoundException()
        {
            ...
            var response = _loggingServiceFixture.LoggingServices.DeleteAllDebugLogAsync(accessToken).Result;
